...
def main():
    ...
    # Monthly: for every company flagged "site?" — a hard crawl error OR a
    # directory/profile-only URL (YC, LinkedIn, …) — find its real website
    # (disambiguated by the observatory topic), then re-crawl + re-classify it
    # in-run so the flag clears and it lands in a real band.
